...
# baseline3의 minmax알고리즘을 활용하고 baseline2 처럼 서로 역할을 유동적으로 바꾸는 agent
...
class activeAgent(BestAgent):
    ...
    def ghost_evaluation(self, currentState, action):
        ...
        if num_opp_pac <= 1:
            ...
            if ((opp_minFoodDist - minFoodDist < 0 or opp_minFoodDist < 3) and minFoodDist < 4) or minGhostDist < 3:
                '''
                otherFoodDist = 0
                for food in foodList:
                    for ghost in self.ghost_opponentState(nextstate):
                        ghostPos = ghost.getPosition()
                        temp = self.getMazeDistance(ghostPos, food)
                        if temp > otherFoodDist:
                            otherFoodDist = temp
                            otherFood = food
                detourFood = self.getMazeDistance(otherFood, newPos)
                '''
                # 단순히 음식만 쫓아가면 먹힐 수도 있으므로 고스트와의 거리 등 여러 요소 고려
                score += 1*minGhostDist  + 1/minFoodDist+1 / avg_FoodDist +(-10) * isstop + (-5) * isrev

                """
                myFoods = self.getFoodYouAreDefending(currentState).asList()
                if len(myFoods) > 0:
...
class minmaxAgent(BestAgent):
    ...
    def evaluationFunction(self, currentgameState):
        ...
        # 안정적인 점수 획득을 위해 음식을 일정량 이상 먹었을 경우 복귀하도록 한다.
    ...
    def getmMWeights(self):
        return {'point': 500, 'minGhostDist': 1 / 15, 'minFoodDist': 10, 'deposit': 20, 'left': -50}
