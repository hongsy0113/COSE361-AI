...
def createTeam(firstIndex, secondIndex, isRed,
               first = 'FlexibleAgent', second = 'FlexibleAgent'):
...
class FlexibleAgent(CaptureAgent):
  ...
  def chooseAction(self, gameState):
    ...
    # 경기 상황에 따라 자신의 역할을 유동적으로 바꾸기 위해서 공격중인 상대 정보 사용
    num_invaders = len(pac_opponents)

    # 둘 중에 누가 수비, 공격을 할 지 정하기 위한 우리팀의 인덱스, 위치 정보
    ...
    # 공격중인 상대가 없다면 둘 다 공격 나갈 수 있도록 공격 평가함수 사용
    if num_invaders == 0:
        scores = [self.attack_evaluate(gameState, action) for action in actions]
    # 공격중인 상대가 한 명이라면 그 상대와 더 가까운 agent가 수비, 다른 하나는 계속 공격
    elif num_invaders ==1:
        ...
        if myDist<= yourDist :
            scores = [self.defense_evaluate(gameState, action) for action in actions]
        else:
            scores = [self.attack_evaluate(gameState, action) for action in actions]
        scores = [self.defense_evaluate(gameState, action) for action in actions]
    # 공격중인 상대가 두 명이라면 두 agent 모두 수비할 수 있도록 수비 평가함수 사용
    elif num_invaders ==2:
        scores = [self.defense_evaluate(gameState, action) for action in actions]
  ...
  # 공격 평가 함수
  def attack_evaluate(self, currentgameState, action):
    #다음상태
    ...
    weights = self.attack_getWeights()
  ...
  def attack_getWeights(self):
    return {'eat': 100, 'DisttoFood': 100, 'DisttoGhost': 1/30, 'num_Food' : -5, 'carrying': 10, 'eatCapsules': 100, 'getpoint' : 500}

  def defense_evaluate(self, currentgameState, action):
    #다음상태
    nextstate = self.getSuccessor(currentgameState, action)
    # 현재위치와 다음위치
    nowPos = currentgameState.getAgentPosition(self.index)
    ...
    score = 0
    features = util.Counter()
    weights = self.defense_getWeights()
  ...
  def defense_getWeights(self):
    return {'notAttack': 10, 'foodsleft': 5, 'DisttoOpponent': -30,'isstop': -10, 'isrev':-10, 'invaders': -20}
