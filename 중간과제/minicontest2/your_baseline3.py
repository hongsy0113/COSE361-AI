...
class minmaxAgent(MyAgent):
    ...
    def evaluationFunction(self, gameState):
        ...
        for e in ghostopp:
            ...
            if temp < minGhostDist:
                minGhostDist = temp
                minGhost = e
        features['deposit'] = gameState.getAgentState(self.index).numCarrying
        features['minGhostDist'] = minGhostDist

        lastState = self.getPreviousObservation()
        getpoint = self.getScore(gameState) - self.getScore(lastState)
    ...
    def getmMWeights(self):
        return {'FoodLeft': -3, 'minFoodDist': 10, 'deposit': 5, 'minGhostDist': 1, 'getpoint':30}
